...
// Cafés sharing the same name (case/whitespace-insensitive) - independent
// of isInternalTestCafe, since a duplicate can be two genuine registrations
// (e.g. a café signing up twice) that neither name pattern nor email would
// ever flag. Read-only: groups for review, nothing deleted here.
app.get("/admin/cafes/duplicates", requireAdminKey, async (req, res) => {
  try {
    const rows = await db
      .prepare(
        "SELECT id, name, email, address, location_address, created_at FROM cafes ORDER BY id ASC",
      )
      .all();
    ...
    res.json({ ok: true, groups: duplicateGroups });
  } catch (err) {
    console.error("Error in /admin/cafes/duplicates:", err);
    res
      .status(500)
      .json({ ok: false, error: String(err && err.message ? err.message : err) });
  }
